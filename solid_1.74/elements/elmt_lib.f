...
integer, intent(in) :: l, nel
integer, intent(out) :: lint
real, intent(out) :: sg(3,*)

integer :: i, j, k, lr(9),lz(9),lw(9)
real :: temp, sqrt_6_10, sqrt_1_3
...
if (l == 1) then
  sg(1,1) = 0.d0
  sg(2,1) = 0.d0
  if (nel == 3) sg(2,1) = -1.0/3.0
  sg(3,1) = 4.0

!     2X2 integration

elseif (l == 2) then
  sqrt_1_3 = 1.0/sqrt(3.0)
  do i = 1, 4
    sg(1,i) = sqrt_1_3*lr(i)
    sg(2,i) = sqrt_1_3*lz(i)
    sg(3,i) = 1.0
  end do ! i
...
elseif (l == 3) then
  ...
  temp = 1.0/81.0
  do i = 1, 9
    sg(1,i) = sqrt_6_10*lr(i)
    sg(2,i) = sqrt_6_10*lz(i)
    sg(3,i) = temp*lw(i)
  end do ! i
...
subroutine int3d(l,lint,sg,nel)

!-----------------------------------------------------------------
!      Purpose: Form Gauss points and weights for three dimensions
...
!      Outputs:
!         lint    - Total number of points
!         sg(4,*) - Array of Gauss points (1:3) and weights (4)
!-----------------------------------------------------------------
...
elseif (l == 2) then
  ...
  sqrt_1_3 = 1.0/sqrt(3.0)
  do i = 1, 4
    sg(1,i)   = ig(i)*sqrt_1_3
    sg(1,i+4) = sg(1,i)
    sg(2,i)   = jg(i)*sqrt_1_3
    sg(2,i+4) = sg(2,i)
    sg(3,i)   =  sqrt_1_3
    sg(3,i+4) = -sqrt_1_3
    sg(4,i)   = 1.0
	...
	STOP 'int3d: quadrature points exceed the maximum(2X2X2)!'
endif
...
real, intent(in) :: ss(2), xl(ndm,*)
logical, intent(in) :: flg
real, intent(out) :: shp(3,*), xsj

integer :: i, j, k
real :: s(4), t(4), xs(3,2), sx(2,2), temp
...
subroutine shp3d(ss,xl,shp,xsj,ndm,nel)
...
!         xl(ndm,*) - Nodal coordinates for element
!         ndm       - Spatial dimension of mesh
!         nel       - Number of nodes attached to element
...
integer, intent(in) :: ndm, nel
real, intent(in) :: ss(3), xl(ndm,*)
real, intent(out) :: shp(4,*), xsj

integer :: i, j, k
real :: rxsj, ap1, am1, ap2, am2, ap3, am3, c1, c2, c3
real :: xs(3,3), ad(3,3)
...
!       Compute for ( + , - ) values
  ...
  STOP 'shp3d: 8-node hexahedron only presently'
endif
...
real :: irad(*)    ,  phi(6,4) ,  theta(3,*),  bbar(2,16,*)
...
real, intent(in) :: aa(6,6)
real, intent(out) :: dd(7,7)
integer :: i, j
