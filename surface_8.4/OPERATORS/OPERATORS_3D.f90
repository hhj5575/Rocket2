...
!        CALL DISTANCE_SURFACE_FACE_HASH(I,TYP1,HASH0,DIR,R,IDX)
    ...
    ALLOCATE(NEW_RIDGE_NUM(NEW_RIDGE_GROUP_NUM), NEW_RIDGE(NEW_RIDGE_GROUP_NUM,1000))
    
    DO I=1,NEW_RIDGE_GROUP_NUM
        READ(io,*) NEW_RIDGE(I,:)
        
        DO J=1,1000
            IF(NEW_RIDGE(I,J+1)==0) THEN
                NEW_RIDGE_NUM(I) = J
                EXIT
            END IF
        END DO
        ...
        DO K=1,3
            DO J1=1,NEW_RIDGE_GROUP_NUM
                DO J2=1,NEW_RIDGE_NUM(J1)-1
                    IF(SURFACE_CURRENT%SURFACE_FACES(K,I) == NEW_RIDGE(J1,J2) .AND. SURFACE_CURRENT%SURFACE_FACES(MOD(K,3)+1,I) == NEW_RIDGE(J1,J2+1)) THEN
                        CALL FACE_NEIGHBOR_FACE(SURFACE_CURRENT%SURFACE_FACES, SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM, SURFACE_CURRENT%POINT_FACE_CONNECTION, I, K, I1)
