    ...
    !$OMP PARALLEL DO PRIVATE(RIDGE_EDGE_NUM,T,L,K, LENGTH, TEMP_INTEGER, INITIAL_LENGTH, RIDGE_EDGE)
    DO I=1,TEMP_POINT_NUM
    ...
    DO J=1,TEMP_CONNECTION_NUM(I)
        ...
        IF(DIVIDED_BOUNDARY_ARRAY(K,L) == BOUNDARY_FLAG) THEN
            TEMP_INTEGER = RIDGE_EDGE_NUM + 1
	    RIDGE_EDGE_NUM = TEMP_INTEGER
            RIDGE_EDGE(1,RIDGE_EDGE_NUM) = K
            RIDGE_EDGE(2,RIDGE_EDGE_NUM) = L
        END IF
    END DO
    ...
    DO I = 1,TEMP_POINT_NUM
        TEMP = TEMPPOINT(:,I) + DISPLACEMENT(:,I)
	TEMPPOINT(:,I) = TEMP
    END DO
    !$OMP END PARALLEL DO
    DEALLOCATE(DISPLACEMENT)
    ...
    IMPLICIT NONE
    REAL(8) :: TIME_STEP, DIST, TEMPDIST
    INTEGER :: I,J,K,J0, IDX, FACE_IDX !I1,I2,I3
...
!real(8) :: t0,t1
    ...
    !$OMP PARALLEL DO PRIVATE(B,K,J0)
    ...
    DO I=1,SURFACE_PROPEL%SURFACE_POINTS_NUM
        ...
        ELSE IF(PROPEL_MOVING_TYPE(I) == 3) THEN
            K = SURFACE_PROPEL%POINT_RELATEDFACE(0+1,I)
            !I1 = SURFACE_FLUID%SURFACE_FACES(1,J)
            !I2 = SURFACE_FLUID%SURFACE_FACES(2,J)
            !I3 = SURFACE_FLUID%SURFACE_FACES(3,J)
            CALL PROJECTION_FACE_POINT_HIGHORDER(SURFACE_PROPEL%SURFACE_POINTS(:,I), K, SURFACE_FLUID%SURFACE_POINTS_NUM, SURFACE_FLUID%SURFACE_POINTS, SURFACE_FLUID%SURFACE_FACES, SURFACE_FLUID%POINT_FACE_CONNECTION_NUM, SURFACE_FLUID%POINT_FACE_CONNECTION, SURFACE_FLUID%POINT_TYPE)
    ...
    !! NOZZLE ABLATION
    !$OMP PARALLEL DO PRIVATE(B,K,IDX)
    DO I=1,SURFACE_CASE%SURFACE_POINTS_NUM
        B = .FALSE.
        ...
        IF(B) THEN
            ...
            IDX = SURFACE_CASE%POINT_RELATEDFACE(0+1,I)
            IF(IDX .NE. 0) THEN
                IF(SURFACE_FLUID%FACE_ABLATION_FLAG(IDX) == -2) THEN
                    !I1 = SURFACE_FLUID%SURFACE_FACES(1,J)
                    !I2 = SURFACE_FLUID%SURFACE_FACES(2,J)
                    !I3 = SURFACE_FLUID%SURFACE_FACES(3,J)
                    
                    CALL PROJECTION_FACE_POINT_HIGHORDER(SURFACE_CASE%SURFACE_POINTS(:,I), IDX, SURFACE_FLUID%SURFACE_POINTS_NUM, SURFACE_FLUID%SURFACE_POINTS, SURFACE_FLUID%SURFACE_FACES, SURFACE_FLUID%POINT_FACE_CONNECTION_NUM, SURFACE_FLUID%POINT_FACE_CONNECTION, SURFACE_FLUID%POINT_TYPE)
    ...
    !$OMP PARALLEL DO PRIVATE(B,C,FACE_IDX,DIST,TEMPDIST,IDX)
    DO I=1,SURFACE_FLUID%SURFACE_POINTS_NUM
        ...
        END DO
        IF(C) THEN
            FACE_IDX = SURFACE_FLUID%POINT_RELATEDFACE(2+1,I)
            IF(FACE_IDX.NE.0) THEN
            
            IF(SURFACE_FLUID%POINT_TYPE(I)==5) THEN
                IF(SURFACE_FLUID%INITIAL_POINT_TYPE(I) .NE. 3) THEN
                    CALL NEIGHBOR_RIDGE_PROJECTION(0,I,2,FACE_IDX)
                ELSE
                    DIST = MAX(DOMAIN_MAX(1)-DOMAIN_MIN(1),DOMAIN_MAX(2)-DOMAIN_MIN(2),DOMAIN_MAX(3)-DOMAIN_MIN(3))
                    DO K=1,3
                        TEMPDIST = SQRT(DOT_PRODUCT(SURFACE_FLUID%SURFACE_POINTS(:,I)-SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(K,FACE_IDX)),SURFACE_FLUID%SURFACE_POINTS(:,I)-SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(K,FACE_IDX))))
                        IF(TEMPDIST<DIST) THEN
                            DIST = TEMPDIST
                            IDX = K
                        END IF
                    END DO
                    SURFACE_FLUID%SURFACE_POINTS(:,I) = SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(IDX,FACE_IDX))
                END IF
            ELSE IF(SURFACE_FLUID%POINT_TYPE(I)==1) THEN

                CALL PROJECTION_FACE_POINT_HIGHORDER(SURFACE_FLUID%SURFACE_POINTS(:,I), FACE_IDX, SURFACE_CASE%SURFACE_POINTS_NUM, SURFACE_CASE%SURFACE_POINTS, SURFACE_CASE%SURFACE_FACES, SURFACE_CASE%POINT_FACE_CONNECTION_NUM, SURFACE_CASE%POINT_FACE_CONNECTION, SURFACE_CASE%POINT_TYPE)
            ...
            ELSE IF(SURFACE_FLUID%POINT_TYPE(I)==6) THEN
                IF(SURFACE_FLUID%INITIAL_POINT_TYPE(I)==3) THEN
                    DIST = MAX(DOMAIN_MAX(1)-DOMAIN_MIN(1),DOMAIN_MAX(2)-DOMAIN_MIN(2),DOMAIN_MAX(3)-DOMAIN_MIN(3))
                    DO K=1,3
                        TEMPDIST = SQRT(DOT_PRODUCT(SURFACE_FLUID%SURFACE_POINTS(:,I)-SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(K,FACE_IDX)),SURFACE_FLUID%SURFACE_POINTS(:,I)-SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(K,FACE_IDX))))
                        IF(TEMPDIST<DIST) THEN
    ...
    !$OMP PARALLEL DO PRIVATE(TEMP_FACE_INDEX)
    DO I=1,TYP1_POINT_NUM
    ...
    STR = './output/surface/source_domain_point_data'// TRIM(ADJUSTL(STR2)) // '.txt'
    write(*,*) TRIM(STR)
    OPEN(UNIT=21, FILE = STR, ACTION = "WRITE", STATUS = "REPLACE")
    DO I=1,TYP1_POINT_NUM
    WRITE(21,'(F, F, F)') TYP1_POINT(1,I), TYP1_POINT(2,I), TYP1_POINT(3,I)
    END DO
    CLOSE(21)
    
    STR = './output/surface/source_domain_face_data'// TRIM(ADJUSTL(STR2)) // '.txt'
    write(*,*) TRIM(STR)
    OPEN(UNIT=21, FILE = STR, ACTION = "WRITE", STATUS = "REPLACE")
    DO I=1,TYP1_FACE_NUM
    WRITE(21,'(I, I, I)') TYP1_FACE(1,I), TYP1_FACE(2,I), TYP1_FACE(3,I)
    END DO
    CLOSE(21)
    
    STR = './output/surface/source_data'// TRIM(ADJUSTL(STR2)) // '.txt'
    write(*,*) TRIM(STR)
    OPEN(UNIT=21, FILE = STR, ACTION = "WRITE", STATUS = "REPLACE")
    DO I=1,TYP1_POINT_NUM
    ...
    WRITE(STR2,*) SURFACE_PRESSURE_ITER
    STR = './output/surface/target_domain_point_data'// TRIM(ADJUSTL(STR2)) // '.txt'
    write(*,*) TRIM(STR)
    OPEN(UNIT=21, FILE = STR, ACTION = "WRITE", STATUS = "REPLACE")
    DO I=1,TYP2_POINT_NUM
    WRITE(21,'(F, F, F)') TYP2_POINT(1,I), TYP2_POINT(2,I), TYP2_POINT(3,I)
    END DO
    CLOSE(21)
    
    STR = './output/surface/target_domain_face_data'// TRIM(ADJUSTL(STR2)) // '.txt'
    write(*,*) TRIM(STR)
    OPEN(UNIT=21, FILE = STR, ACTION = "WRITE", STATUS = "REPLACE")
    DO I=1,TYP2_FACE_NUM
    WRITE(21,'(I, I, I)') TYP2_FACE(1,I), TYP2_FACE(2,I), TYP2_FACE(3,I)
    END DO
    CLOSE(21)
    ...
    STR = './output/surface/target_data'// TRIM(ADJUSTL(STR2)) // '.txt'
    write(*,*) TRIM(STR)
    OPEN(UNIT=21, FILE = STR, ACTION = "WRITE", STATUS = "REPLACE")
    DO I=1,TYP2_POINT_NUM
    WRITE(21,'(F)') CR_TARGET(I)
    END DO
    CLOSE(21)
    ...
    !$OMP PARALLEL DO PRIVATE(I1,I2,I3,I4,POINT_I1,POINT_I2,POINT_I3,POINT_I4,TEMP_NORMAL1,TEMP_NORMAL2)
    DO I=1,FACE_NUM
    ...
    END SUBROUTINE NEIGHBOR_RIDGE_PROJECTION
        ...
        REAL(8), ALLOCATABLE :: DISPLACEMENT(:,:), DISPLACEMENT_THRESHOLD(:), DISPLACEMENT_TEMP(:,:)
        
        REAL(8), ALLOCATABLE :: FACE_AREA(:)
        REAL(8), ALLOCATABLE :: FACE_CENTER(:,:)
        
        INTEGER :: I,J,K,L,M, IDX, POINT_IDX, IDX2, IDX3
        INTEGER :: I0,I1,I2,I3, J1,J2,J3,J4,J5,J6,J7, N1,N2,N3, TEMP_I 
        REAL(8) :: V(3), V1(3),V2(3),R,TEMPVEC(3),TEMP_NORMAL(3),L1,L2
        ...
        REAL(8), ALLOCATABLE :: W_SUM(:)
        REAL(8), ALLOCATABLE :: B(:,:), B_TEMP(:,:)
        
        REAL(8) :: DUMMY(3,3)
        REAL(8), ALLOCATABLE :: E_VECTOR(:,:,:), E_VECTOR_TEMP(:,:,:), E_VALUE(:,:)
        ...
        REAL(8) :: MU_SUM
        LOGICAL :: EXPANDING, FLAG, FLAG1, FLAG2, FLAG3, B0, B1, B2, B5 !B2, B3, B4, B5
        LOGICAL, ALLOCATABLE :: POINT_USED(:)
        REAL(8) :: D(3),DIR(3)
       
        REAL(8) :: PHI_C, PHI_R, CHI_C, CHI_R
	REAL(8),ALLOCATABLE :: THETA_A(:)
	REAL(8),ALLOCATABLE :: THETA_A_TEMP(:)
	REAL(8) :: DIST1, DIST2 !POINT_DIST, MINDIST, MINDIST1, MINDIST2, DIST3, DISTANCE
    
        INTEGER :: RIDGE_NUM, FACE_IDX, FACE_INDEX, FACE_INDEX2, VERTEX, VERTEX_IDX, NUM, NEIGHBOR_INDEX, POINT_IDX1, POINT_IDX2, POINT_IDX3, POINT_IDX4, POINT_IDX5 !RIDGE(2)
        REAL(8) :: NEIGHBOR_RIDGE_POINT(3,2), INNER, INNER1, INNER2, CRI, TEMPDIST,A1(3), A2(3),T(3), CENTER1(3), CENTER2(3), S
	...
	ALLOCATE(W_SUM(SURFACE_CURRENT%SURFACE_POINTS_NUM))
	ALLOCATE(THETA_A(SURFACE_CURRENT%SURFACE_POINTS_NUM))
        W_SUM = 0.
        THETA_A = 0.

	!$OMP PARALLEL DO PRIVATE(I1,I2,I3,V1,V2,R)
	...
	ALLOCATE(THETA_A_TEMP(SURFACE_CURRENT%SURFACE_POINTS_NUM))
	THETA_A_TEMP = 0.

	!$OMP PARALLEL DO PRIVATE(I1,I2,I3,TEMP_I,V1,V2)
        DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM                     
	    DO J = 1,SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(I)
                W_SUM(I) = W_SUM(I) + FACE_AREA(SURFACE_CURRENT%POINT_FACE_CONNECTION(J,I))
                ...
                V1 = SURFACE_CURRENT%SURFACE_POINTS(:,I2) - SURFACE_CURRENT%SURFACE_POINTS(:,I1)
                V2 = SURFACE_CURRENT%SURFACE_POINTS(:,I3) - SURFACE_CURRENT%SURFACE_POINTS(:,I1)
                THETA_A_TEMP(I) = THETA_A_TEMP(I) + ACOS(MAX(-1., MIN(1., DOT_PRODUCT(V1,V2)/SQRT(DOT_PRODUCT(V1,V1) * DOT_PRODUCT(V2,V2)) )) )
            END DO
	    !$OMP ATOMIC
	    THETA_A(I) = THETA_A(I) + THETA_A_TEMP(I) 
	END DO
	!$OMP END PARALLEL DO

	DEALLOCATE(THETA_A_TEMP)

        ALLOCATE(W(30,SURFACE_CURRENT%SURFACE_POINTS_NUM))
	ALLOCATE(A(30,SURFACE_CURRENT%SURFACE_POINTS_NUM))
	ALLOCATE(N(30,3,SURFACE_CURRENT%SURFACE_POINTS_NUM))
	...
	DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM
	    DO J = 1,SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(I)
                N(J,:,I) = NORMAL(:,SURFACE_CURRENT%POINT_FACE_CONNECTION(J,I))
                W(J,I) = FACE_AREA(SURFACE_CURRENT%POINT_FACE_CONNECTION(J,I)) / W_SUM(I)
                A(J,I) = TIMESTEP * SURFACE_CURRENT%FACE_B_RATE(SURFACE_CURRENT%POINT_FACE_CONNECTION(J,I))
            END DO
        END DO
	!$OMP END PARALLEL DO

	ALLOCATE(E_VECTOR(3,3,SURFACE_CURRENT%SURFACE_POINTS_NUM))
	ALLOCATE(E_VALUE(3,SURFACE_CURRENT%SURFACE_POINTS_NUM))
	ALLOCATE(B(3,SURFACE_CURRENT%SURFACE_POINTS_NUM))

	E_VECTOR = 0.
	E_VALUE = 0.
	B = 0.

	ALLOCATE(E_VECTOR_TEMP(3,3,SURFACE_CURRENT%SURFACE_POINTS_NUM))
	ALLOCATE(B_TEMP(3,SURFACE_CURRENT%SURFACE_POINTS_NUM))
	...
	DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM	
            DO K=1,3
                DO L=1,3
                    DO J = 1,SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(I)
                        E_VECTOR_TEMP(K,L,I) = E_VECTOR_TEMP(K,L,I) + W(J,I)*N(J,K,I)*N(J,L,I)
                    END DO
                END DO

                DO J = 1,SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(I)
                    B_TEMP(K,I) = B_TEMP(K,I) + W(J,I)*N(J,K,I)*A(J,I)
                END DO
            END DO 
                !$OMP ATOMIC
		E_VECTOR(1,1,I) = E_VECTOR(1,1,I) + E_VECTOR_TEMP(1,1,I)
                !$OMP ATOMIC
		E_VECTOR(1,2,I) = E_VECTOR(1,2,I) + E_VECTOR_TEMP(1,2,I)
                !$OMP ATOMIC
		E_VECTOR(1,3,I) = E_VECTOR(1,3,I) + E_VECTOR_TEMP(1,3,I)
                !$OMP ATOMIC
		E_VECTOR(2,1,I) = E_VECTOR(2,1,I) + E_VECTOR_TEMP(2,1,I)
                !$OMP ATOMIC
		E_VECTOR(2,2,I) = E_VECTOR(2,2,I) + E_VECTOR_TEMP(2,2,I)
                !$OMP ATOMIC
		E_VECTOR(2,3,I) = E_VECTOR(2,3,I) + E_VECTOR_TEMP(2,3,I)
                !$OMP ATOMIC
		E_VECTOR(3,1,I) = E_VECTOR(3,1,I) + E_VECTOR_TEMP(3,1,I)
                !$OMP ATOMIC
		E_VECTOR(3,2,I) = E_VECTOR(3,2,I) + E_VECTOR_TEMP(3,2,I)
                !$OMP ATOMIC
		E_VECTOR(3,3,I) = E_VECTOR(3,3,I) + E_VECTOR_TEMP(3,3,I)

                !$OMP ATOMIC
		B(1,I) = B(1,I) + B_TEMP(1,I)
                !$OMP ATOMIC
		B(2,I) = B(2,I) + B_TEMP(2,I)
                !$OMP ATOMIC
		B(3,I) = B(3,I) + B_TEMP(3,I)
        END DO
	!$OMP END PARALLEL DO

	DEALLOCATE(E_VECTOR_TEMP)
	DEALLOCATE(B_TEMP)
	
	DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM              
            CALL SVDCMP_ROUTINE(E_VECTOR(:,:,I),3,3,3,3,E_VALUE(:,I),DUMMY)
	END DO

	ALLOCATE(LOCAL_SHAPE(SURFACE_CURRENT%SURFACE_POINTS_NUM))
	LOCAL_SHAPE = 0
 	ALLOCATE(DISPLACEMENT_TEMP(3, SURFACE_CURRENT%SURFACE_POINTS_NUM))
	DISPLACEMENT_TEMP = 0.
	!$OMP DO ORDERED 
	DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM   
	    !$OMP ORDERED
            DO J=1,2
                JMIN = J
                DO K=J+1,3
                    IF(E_VALUE(K,I)>E_VALUE(JMIN,I)) THEN
                        JMIN = K
                    END IF
                END DO
                
                IF(J .NE. JMIN) THEN
                    TEMP_E_VALUE = E_VALUE(J,I)
                    TEMP_E_VECTOR = E_VECTOR(:,J,I)
                    
                    E_VALUE(J,I) = E_VALUE(JMIN,I)
                    E_VECTOR(:,J,I) = E_VECTOR(:,JMIN,I)
                    
                    E_VALUE(JMIN,I) = TEMP_E_VALUE
                    E_VECTOR(:,JMIN,I) = TEMP_E_VECTOR(:)
                END IF
		!$OMP FLUSH(E_VALUE, E_VECTOR, JMIN)
            END DO              
            !$OMP END ORDERED
            THETA_A(I) = THETA_A(I) - 2.*PI
                  
            IF(E_VALUE(3,I)/E_VALUE(1,I) > CHI_C .OR. ABS(THETA_A(I)) >= PI/4.) THEN
                LOCAL_SHAPE(I) = 3
            ELSE IF(E_VALUE(2,I)/E_VALUE(1,I) > CHI_R) THEN
                LOCAL_SHAPE(I) = 2
            ELSE
                LOCAL_SHAPE(I) = 1
            END IF
            ...
            LOCAL_SHAPE(I) = SURFACE_CURRENT%INITIAL_POINT_TYPE(I)
    
	    DO J = 1, LOCAL_SHAPE(I)
                DISPLACEMENT_TEMP(:,I) = DISPLACEMENT_TEMP(:,I) + DOT_PRODUCT(E_VECTOR(:,J,I),B(:,I))/E_VALUE(J,I) * E_VECTOR(:,J,I)
	    END DO 
	    !$OMP ATOMIC
	    DISPLACEMENT(1,I) = DISPLACEMENT(1,I) + DISPLACEMENT_TEMP(1,I)
	    !$OMP ATOMIC
	    DISPLACEMENT(2,I) = DISPLACEMENT(2,I) + DISPLACEMENT_TEMP(2,I)
	    !$OMP ATOMIC
	    DISPLACEMENT(3,I) = DISPLACEMENT(3,I) + DISPLACEMENT_TEMP(3,I)
        END DO
	!$OMP END DO
	DEALLOCATE(DISPLACEMENT_TEMP)
	DEALLOCATE(W,A,N)
	...
	IF(SURFACE_CURRENT%POINT_TYPE(I)==1) THEN
		DO J = 1,LOCAL_SHAPE(I)
	            DISPLACEMENT(:,I) = DISPLACEMENT(:,I) + DOT_PRODUCT(E_VECTOR(:,J,I),B(:,I))/E_VALUE(J,I) * E_VECTOR(:,J,I)
		END DO
	END IF
	END IF
	END DO
...
!$OMP PARALLEL DO PRIVATE(K,FLAG1,FLAG2,FLAG3,TEMP,TEMPFACE,V1,V2,TEMP_NORMAL,TEMPVEC,INNER1,INNER2)
DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM 
IF(SQRT(DOT_PRODUCT(DISPLACEMENT(:,I),DISPLACEMENT(:,I)))>0) THEN
...
!$OMP PARALLEL DO PRIVATE(FLAG,K,V1,V2,TEMPVEC,INNER1,INNER2)
	DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM 
IF(SQRT(DOT_PRODUCT(DISPLACEMENT(:,I),DISPLACEMENT(:,I)))>0) THEN
...
!$OMP PARALLEL DO PRIVATE(RELATED_FACE,B1,POINT_IDX,IDX,J1,J2,J3,RIDGE_NUM,N1,N2,N3,POINT_IDX1,POINT_IDX2,DIR1,DIR2,NUM,TEMP_NORMAL2,K,W1,W2,INNER1,INNER2,DIR,POINT_IDX3,TEMPVEC,TEMP_NORMAL,POINT_IDX4,B5,FLAG,FACE_IDX,POINT_IDX5,NEIGHBOR_RIDGE_POINT,V1,L1,V2,L2,V,CENTER1,RELATED_NORMAL,DIST,B0,VERTEX,TEMP_DIST,TEMP_NEIGHBOR_NORMAL,CRI,NEIGHBOR_NORMAL,NEIGHBOR_INDEX,CENTER2,DIST1,DIST2,RIDGE_EDGE,POINT_USED,FACE_INDEX,I0,TEMPDIST,J4,J5,J6,J7,RIDGE_EDGE_NUM,A1,A2,POINTDIST,INNER,T,R)
DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM 
IF(SQRT(DOT_PRODUCT(DISPLACEMENT(:,I),DISPLACEMENT(:,I)))>0) THEN
IF(SURFACE_CURRENT%POINT_TYPE(I)==6) THEN
	...
	IF(.NOT. B1 .AND. RIDGE_NUM == 1) THEN
        ...
        ELSEIF(B1) THEN
            ...
            POINT_IDX4 = SURFACE_CASE%SURFACE_FACES(IDX,RELATED_FACE)
            NUM = 0           
            DO J = 1, SURFACE_CASE%POINT_FACE_CONNECTION_NUM(POINT_IDX4)     
                K = SURFACE_CASE%POINT_FACE_CONNECTION(J,POINT_IDX4)
                ...
                DO J = 1, SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(I)
                    ...
                    DO K=1,3
                        IF(SURFACE_CURRENT%SURFACE_FACES(K,FACE_IDX)==I) THEN
                            POINT_IDX5 = SURFACE_CURRENT%SURFACE_FACES(MOD(K,3)+1,FACE_IDX)
                            IF(SURFACE_CURRENT%FACE_DIVIDED_BOUNDARY_ARRAY(K,FACE_IDX) .NE. 0 .AND. SURFACE_CURRENT%POINT_TYPE(POINT_IDX5)==4) THEN
                                FLAG = .TRUE.   
                                EXIT
                            END IF
                        END IF
            ...
            RIDGE_NUM = 0
            NEIGHBOR_RIDGE_POINT(:,1) = SURFACE_CURRENT%SURFACE_POINTS(:,POINT_IDX5)
            NEIGHBOR_RIDGE_POINT(:,2) = SURFACE_CURRENT%SURFACE_POINTS(:,POINT_IDX5)
        
            DO J = 1, SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(POINT_IDX5)
                FACE_IDX = SURFACE_CURRENT%POINT_FACE_CONNECTION(J,POINT_IDX5)
                
                DO K=1,3
                    IF(SURFACE_CURRENT%SURFACE_FACES(K,FACE_IDX)==POINT_IDX5) THEN
                        EXIT
                    END IF
                END DO
            ...
            IF(RIDGE_NUM < 2) THEN
                ...
                WRITE(*,*) 'INDICES : ', POINT_IDX5
            END IF           

            V1 = NEIGHBOR_RIDGE_POINT(:,1) - SURFACE_CURRENT%SURFACE_POINTS(:,POINT_IDX5)
            L1 = SQRT(DOT_PRODUCT(V1,V1))
            V2 = -(NEIGHBOR_RIDGE_POINT(:,2) - SURFACE_CURRENT%SURFACE_POINTS(:,POINT_IDX5))
            L2 = SQRT(DOT_PRODUCT(V2,V2))        

            V = L1*V1 + L2*V2
            V = V/SQRT(DOT_PRODUCT(V,V))

            RELATED_FACE = SURFACE_CURRENT%POINT_RELATEDFACE(2+1,POINT_IDX5)
            ...
            DO J = 1, 3
                ...
                DO L = 1, NUM
                    ...
                    CALL UNSIGNED_DISTANCE_FACE_POINT(SURFACE_CURRENT%SURFACE_POINTS(:,POINT_IDX5), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(1,K)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(2,K)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(3,K)), TEMP_DIST)	
            ...
            ELSE
                IF(B0) THEN
                    IF(DOT_PRODUCT(CENTER1-SURFACE_CURRENT%SURFACE_POINTS(:,POINT_IDX5),DIR1)<0) THEN
                        DIR1 = -DIR1
                    END IF
                    DIR2 = DIR2/SQRT(DOT_PRODUCT(DIR2,DIR2))
                    IF(DOT_PRODUCT(CENTER2-SURFACE_CURRENT%SURFACE_POINTS(:,POINT_IDX5),DIR2)<0) THEN
                        DIR2 = -DIR2
                    END IF
                    
                    NUM = SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(POINT_IDX5)
                    DO L = 1, NUM
                        K = SURFACE_CURRENT%POINT_FACE_CONNECTION(L,POINT_IDX5)
                        IF(SURFACE_CURRENT%FACE_ONINTERFACE(K)==1) THEN
                            W1 = SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(2,K)) - SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(1,K))
                            W2 = SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(3,K)) - SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(1,K))
                            CALL VEC_CURL1(W1,W2,TEMP_NORMAL)  
...
!$OMP PARALLEL DO PRIVATE(RIDGE_NUM,NEIGHBOR_RIDGE_POINT,FACE_IDX,V1,L1,V2,L2,V,RELATED_FACE,B2,VERTEX_IDX,FACE_INDEX2,CENTER1,RELATED_NORMAL,DIST,B0,VERTEX,NUM,TEMP_DIST,W1,W2,TEMP_NEIGHBOR_NORMAL,CRI,NEIGHBOR_INDEX,TEMP_NORMAL,CENTER2,DIR,DIR1,DIR2,IDX,IDX2,IDX3,INNER1,INNER2)
DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM
 IF(SURFACE_CURRENT%POINT_TYPE(I)==4) THEN 
		...
		END IF
            ...
            IF(SURFACE_CASE%FACE_ONINTERFACE(RELATED_FACE) .NE. 1) THEN
                DO J=1,3
                VERTEX_IDX = SURFACE_CASE%SURFACE_FACES(J,RELATED_FACE)
                    DO K=1,SURFACE_CASE%POINT_FACE_CONNECTION_NUM(VERTEX_IDX)
                        FACE_INDEX2 = SURFACE_CASE%POINT_FACE_CONNECTION(K,VERTEX_IDX)
                        IF(SURFACE_CASE%FACE_ONINTERFACE(FACE_INDEX2)==1) THEN
                            RELATED_FACE = FACE_INDEX2
                            B2 = .TRUE.
                            EXIT
                        END IF                      
                    END DO
            ...
            DO J = 1, 3
                ...
                NUM = SURFACE_CASE%POINT_FACE_CONNECTION_NUM(VERTEX)
                DO L = 1, NUM
                    IDX = SURFACE_CASE%POINT_FACE_CONNECTION(L,VERTEX)
                    CALL UNSIGNED_DISTANCE_FACE_POINT(SURFACE_CURRENT%SURFACE_POINTS(:,I), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(1,IDX)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(2,IDX)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(3,IDX)), TEMP_DIST)	

                    W1 = SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(2,IDX)) - SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(1,IDX))  
                    W2 = SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(3,IDX)) - SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(1,IDX)) 
                    CALL VEC_CURL1(W1,W2,TEMP_NEIGHBOR_NORMAL)
                    TEMP_NEIGHBOR_NORMAL = TEMP_NEIGHBOR_NORMAL/SQRT(DOT_PRODUCT(TEMP_NEIGHBOR_NORMAL,TEMP_NEIGHBOR_NORMAL))
                    CRI = SQRT(MAX(0.,1.-DOT_PRODUCT(RELATED_NORMAL,TEMP_NEIGHBOR_NORMAL)*DOT_PRODUCT(RELATED_NORMAL,TEMP_NEIGHBOR_NORMAL)))

                    IF(IDX/=RELATED_FACE .AND. TEMP_DIST<DIST .AND. CRI>0.5 .AND. SURFACE_CASE%FACE_ONINTERFACE(IDX) .NE. -1 ) THEN
                        B0 = .TRUE.
                        DIST = TEMP_DIST
                        NEIGHBOR_NORMAL = TEMP_NEIGHBOR_NORMAL	
                        NEIGHBOR_INDEX = IDX

                        CENTER2 = (SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(1,IDX))+SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(2,IDX))+SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(3,IDX)))/3.
                    END IF 
            ...
            ELSE
                IF(B0) THEN
                    ...
                    NUM = SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(I)
                    DO L = 1, NUM
                        IDX2 = SURFACE_CURRENT%POINT_FACE_CONNECTION(L,I)
                        IF(SURFACE_CURRENT%FACE_ONINTERFACE(IDX2)==1) THEN
                            W1 = SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(2,IDX2)) - SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(1,IDX2))
                            W2 = SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(3,IDX2)) - SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(1,IDX2))
                            CALL VEC_CURL1(W1,W2,TEMP_NORMAL)  
                            INNER1 = DOT_PRODUCT(TEMP_NORMAL,DIR1)
                            INNER2 = DOT_PRODUCT(TEMP_NORMAL,DIR2)
                            IF(INNER1>0. .AND. INNER1>=INNER2) THEN
                ...
                ELSE
                    ...
                    NUM = SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(I)
                    DO L = 1, NUM
                        IDX3 = SURFACE_CURRENT%POINT_FACE_CONNECTION(L,I)
                        IF(SURFACE_CURRENT%FACE_ONINTERFACE(IDX3)==1) THEN
                            W1 = SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(2,IDX3)) - SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(1,IDX3))
                            W2 = SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(3,IDX3)) - SURFACE_CURRENT%SURFACE_POINTS(:,SURFACE_CURRENT%SURFACE_FACES(1,IDX3))
                            CALL VEC_CURL1(W1,W2,TEMP_NORMAL)  
                            INNER1 = DOT_PRODUCT(TEMP_NORMAL,DIR1)
                            INNER2 = DOT_PRODUCT(TEMP_NORMAL,DIR2)
                            IF(INNER1>0. .AND. INNER1>=INNER2) THEN
...
END DO
!$OMP END PARALLEL DO
!$OMP PARALLEL DO PRIVATE(K,FLAG,V1,V2,TEMPVEC,INNER1,INNER2)
DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM 
...
!$OMP END PARALLEL DO
	...
	ALLOCATE(A(30,SURFACE_CURRENT%SURFACE_POINTS_NUM))
        ALLOCATE(W(30,SURFACE_CURRENT%SURFACE_POINTS_NUM))
        ALLOCATE(MU(30,SURFACE_CURRENT%SURFACE_POINTS_NUM))
        ALLOCATE(LL(30,SURFACE_CURRENT%SURFACE_POINTS_NUM))
        ALLOCATE(COSTHETA(30,SURFACE_CURRENT%SURFACE_POINTS_NUM))
	...
	!$OMP PARALLEL DO PRIVATE(POINT_USED,EDGE_LENGTH,MU_SUM,I1,I2,EXPANDING,D,DISPLACEMENT_THRESHOLD1,DISPLACEMENT_THRESHOLD2,FACE_INDEX)  
        DO I = 1,SURFACE_CURRENT%SURFACE_POINTS_NUM
	    ...
	    EDGE_LENGTH = MAX(DOMAIN_MAX(1)-DOMAIN_MIN(1),DOMAIN_MAX(2)-DOMAIN_MIN(2),DOMAIN_MAX(3)-DOMAIN_MIN(3))
            ...
            ELSE
                ...
                DO J = 1,SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(I)
                    W(J,I) = FACE_AREA(SURFACE_CURRENT%POINT_FACE_CONNECTION(J,I))
                    A(J,I) = TIMESTEP * SURFACE_CURRENT%FACE_B_RATE(SURFACE_CURRENT%POINT_FACE_CONNECTION(J,I))
                    COSTHETA(J,I) = DOT_PRODUCT(DISPLACEMENT(:,I), NORMAL(:,SURFACE_CURRENT%POINT_FACE_CONNECTION(J,I))) &
			...
			END IF
                    ...
                    IF(EXPANDING) THEN
                        ...
                        LL(J,I) = A(J,I)
                    ELSE
                        MU(J,I) = W(J,I) * (COSTHETA(J,I))**2
                        
                        IF(ABS(MU(J,I))<MINERROR .OR. 1./ABS(COSTHETA(J,I)) > 20.) THEN
                            LL(J,I) = 0.
                        ELSE
                            LL(J,I) = A(J,I) / ABS(COSTHETA(J,I))
                        END IF
                    END IF
                    
                    IF(ABS(LL(J,I))>=MINERROR) THEN
                        MU_SUM = MU_SUM + MU(J,I)
                        LENGTH(I) = LENGTH(I) + MU(J,I)*LL(J,I)
			DISPLACEMENT_THRESHOLD2 = MAX(DISPLACEMENT_THRESHOLD2,LL(J,I))
                    END IF
	...
	!$OMP PARALLEL DO PRIVATE(R,S)
        DO I = 1, SURFACE_CURRENT%SURFACE_POINTS_NUM
            R = 0.
	    S = SQRT(DOT_PRODUCT(DISPLACEMENT(:,I),DISPLACEMENT(:,I)))
	    !!$OMP CRITICAL
            IF(S>=1E-16) THEN
...
write(*,*) 'RESULT_TIMESTEP :', result_timestep
    ...
    INTEGER, ALLOCATABLE :: A_INDICES_CG(:,:) , A_INDICES_NUM_CG(:)
    ...
    DT = 0.
    !$OMP PARALLEL DO PRIVATE(I1, TEMP)
    DO I=1,TEMP_POINT_NUM
        DO J=1,TEMP_CONNECTION_NUM(I)
            I1 = TEMP_CONNECTION(J,I)
            TEMP = DT(I) + SQRT(SQRT(3.) * TEMP_NEWFACEAREA(I1))/REAL(TEMP_CONNECTION_NUM(I))
	    DT(I) = TEMP
        END DO
    END DO
    !$OMP END PARALLEL DO  
    ...
    X_N(:,:) = TEMPPOINT(:,:)
    X_N1(:,:) = 0.
    ALLOCATE(A_CG(POINT_INDEX_NUM*3,94), A_INDICES_CG(POINT_INDEX_NUM*3,94), A_INDICES_NUM_CG(POINT_INDEX_NUM*3), B_CG(POINT_INDEX_NUM*3), X_CG(POINT_INDEX_NUM*3))
    ...
    !$OMP PARALLEL DO PRIVATE(NUM, j1)
    DO I=1,TEMP_POINT_NUM
        ...
        A_INDICES_NUM_CG(POINT_INDEX(I)) = NUM
        A_INDICES_NUM_CG(POINT_INDEX_NUM + POINT_INDEX(I)) = NUM
        A_INDICES_NUM_CG(2*POINT_INDEX_NUM + POINT_INDEX(I)) = NUM
    ...
    CALL CG_SOLVER_SPARSE(A_CG, A_INDICES_CG,A_INDICES_NUM_CG, B_CG, POINT_INDEX_NUM*3, X_CG)
    ELSE
    ...
    DEALLOCATE(A_CG, A_INDICES_CG, B_CG, X_CG, A_INDICES_NUM_CG)
    ...
    REAL(8), ALLOCATABLE :: W_SUM(:), W_SUM_TEMP(:), E_VECTOR(:,:,:), E_VALUE(:,:), E_VECTOR_TEMP(:,:,:)
    REAL(8) :: R
    REAL(8) :: DUMMY(3,3)
    REAL(8) :: TEMP_E_VALUE, TEMP_E_VECTOR(3)
    REAL(8) :: TT(3,3), TEMP_NORMAL(3), TEMP_REAL
    ...
    DISPLACEMENT(:,:) = 0.
    !$OMP PARALLEL DO PRIVATE(V1,V2,R,TEMP_NORMAL)
    DO I=1,TEMP_FACE_NUM
        V1 = TEMPPOINT(:,TEMPFACE(2,I)) - TEMPPOINT(:,TEMPFACE(1,I))
        V2 = TEMPPOINT(:,TEMPFACE(3,I)) - TEMPPOINT(:,TEMPFACE(1,I))
        ...
        R = SQRT(DOT_PRODUCT(NORMAL(:,I), NORMAL(:,I)))
        FACE_AREA(I) = R/2.
	TEMP_NORMAL = NORMAL(:,I)/R
        NORMAL(:,I) = TEMP_NORMAL
    END DO
    !$OMP END PARALLEL DO

    ALLOCATE(W(30,TEMP_POINT_NUM))
    ALLOCATE(N(30,3,TEMP_POINT_NUM))
    ALLOCATE(W_SUM(TEMP_POINT_NUM))
    ALLOCATE(W_SUM_TEMP(TEMP_POINT_NUM))
    ALLOCATE(E_VECTOR(3,3,TEMP_POINT_NUM))
    ALLOCATE(E_VECTOR_TEMP(3,3,TEMP_POINT_NUM))
    ALLOCATE(E_VALUE(3,TEMP_POINT_NUM))
    ALLOCATE(T(3,3))
    W = 0
    N = 0
    W_SUM = 0
    ...
    !$OMP PARALLEL DO PRIVATE(TEMP_REAL)
    DO I=1,TEMP_POINT_NUM
        IF(SQRT(DOT_PRODUCT(BEFORE_DISPLACEMENT(:,I), BEFORE_DISPLACEMENT(:,I))) > MINERROR) THEN
            DO J = 1,TEMP_CONNECTION_NUM(I)
                TEMP_REAL = W_SUM(I) + FACE_AREA(TEMP_CONNECTION(J,I))
		W_SUM(I) = TEMP_REAL
            END DO

            DO J = 1,TEMP_CONNECTION_NUM(I)
                N(J,:,I) = NORMAL(:,TEMP_CONNECTION(J,I))
                W(J,I) = FACE_AREA(TEMP_CONNECTION(J,I)) / W_SUM(I)
            END DO

            DO K=1,3
                DO L=1,3
                    DO J = 1,TEMP_CONNECTION_NUM(I)
                        TEMP_REAL = E_VECTOR(K,L,I) + W(J,I)*N(J,K,I)*N(J,L,I)
			E_VECTOR(K,L,I) = TEMP_REAL
                    END DO
                END DO
            END DO
            ! COMPUTING EIGEN VALUE & EIGEN VECTOR OF LS_MATRIX
    ...
    !$OMP PARALLEL DO PRIVATE(JMIN, TEMP_E_VALUE, TEMP_E_VECTOR,T,TT, LOCAL_SHAPE, TEMP_REAL)
    DO I = 1, TEMP_POINT_NUM
	IF (SQRT(DOT_PRODUCT(BEFORE_DISPLACEMENT(:,I),BEFORE_DISPLACEMENT(:,I)))>MINERROR) THEN
            DO J=1,3-1
                JMIN = J
	        ...
	        T = 0
                ...
                DO K = 1,3
                    DO L = 1,3
                        TEMP_REAL = DISPLACEMENT(K,I) + TT(K,L)*BEFORE_DISPLACEMENT(L,I)
			DISPLACEMENT(K,I) = TEMP_REAL
                    END DO
                END DO
            END IF
        END IF
...
!$OMP END PARALLEL DO
    ...
    !$OMP PARALLEL DO PRIVATE(I,V1,V2,V3,V4,R1,R2,F_SIZE_SHAPE,F_SIZE,F_SHAPE,F_SIZE_SHAPE1,F_SIZE1,F_SHAPE1,F_SIZE_SHAPE_NEW,F_SIZE_NEW,F_SHAPE_NEW,F_SIZE_SHAPE1_NEW,F_SIZE1_NEW,F_SHAPE1_NEW,N,N_NEW,N1,N1_NEW,QUALITY_BEFORE,QUALITY_AFTER)
    DO I=1,FACE_NUM
        V1 = POINT(:,FACE(1,I))
        V2 = POINT(:,FACE(2,I))
        V3 = POINT(:,FACE(3,I))
	...
	CALL VEC_CURL2(V1,V2,V1,V3,N)
        CALL VEC_CURL2(V1,V3,V1,V4, N1)
        ...
        CALL VEC_CURL2(V2,V3,V2,V4, N_NEW)
        CALL VEC_CURL2(V2,V4,V2,V1, N1_NEW)
