MODULE HIGHORDER_3D
    ...
    SUBROUTINE MATMUL_SPARSE(A,A_INDICES,A_INDICES_NUM,B,N, C)
	IMPLICIT NONE
        REAL(8) :: A(:,:)
        INTEGER :: A_INDICES(:,:)
        INTEGER :: A_INDICES_NUM(:)
	...
	REAL(8) :: TEMP
        ...
        DO I=1,N
            C(I) = 0.         
            DO J=1,A_INDICES_NUM(I)
                TEMP = C(I) + A(I,J) * B(A_INDICES(I,J))
		C(I) = TEMP
            END DO
	END DO
	!$OMP END PARALLEL DO 
    ...
    SUBROUTINE CG_SOLVER_SPARSE(A, A_INDICES, A_INDICES_NUM,B, N, X)
	IMPLICIT NONE
        REAL(8) :: A(:,:)
        INTEGER :: A_INDICES(:,:)
        INTEGER :: A_INDICES_NUM(:)
        ...
        CALL MATMUL_SPARSE(A,A_INDICES,A_INDICES_NUM,X,N,AD)
        ...
        DO K = 1, MAX_ITER
            ...
            CALL MATMUL_SPARSE(A,A_INDICES,A_INDICES_NUM,D,N,AD)
    ...
    SUBROUTINE RECONSTRUCTING_HIGH_ORDER_SURFACE(POINT_NUM, POINT, FACE, FACE_INDEX, V, POLY_ORDER, RING_NUM, CONNECTION_NUM, CONNECTION, POINT_TYPE, V_RESULT)
        ...
        INTEGER, ALLOCATABLE :: POINT_INDEX_CLUSTER(:), CURRENT_CLUSTER(:), TEMP_CLUSTER(:)
        REAL(8), ALLOCATABLE :: POINT_CLUSTER(:,:)
        INTEGER :: POINT_CLUSTER_NUM, CURRENT_CLUSTER_NUM, TEMP_CLUSTER_NUM
        ...
        ALLOCATE(POINT_INDEX_CLUSTER(POINT_NUM), USED_POINT(POINT_NUM), CURRENT_CLUSTER(POINT_NUM), TEMP_CLUSTER(POINT_NUM))
        ...
        DO ITER=1,NEIGHBOR_NUM
            ...
            USED_POINT(NEIGHBOR(ITER)) = .TRUE.
            
            CURRENT_CLUSTER(:) = 0
            CURRENT_CLUSTER_NUM = 1
            CURRENT_CLUSTER(1) = NEIGHBOR(ITER)
            
            DO I=1,RING_NUM
                TEMP_CLUSTER_NUM = 0
                TEMP_CLUSTER(:) = 0
                
                DO J=1,CURRENT_CLUSTER_NUM
                    IF(POINT_TYPE(CURRENT_CLUSTER(J))==1) THEN
                    
                    DO K=1,CONNECTION_NUM(CURRENT_CLUSTER(J))
                        ...
                        IF(.NOT. USED_POINT(NEIGHBOR_POINT)) THEN
                            ...
                            TEMP_CLUSTER(TEMP_CLUSTER_NUM) = NEIGHBOR_POINT
                        END IF
                    END DO
            ...
            CALL LOCAL_POLY_FITTING_POINT(POLY_ORDER, POINT_CLUSTER_NUM, POINT_CLUSTER, NEW_V_TEMP, FITTING)
        ...
        DEALLOCATE(POINT_INDEX_CLUSTER, USED_POINT, CURRENT_CLUSTER, TEMP_CLUSTER)
        
    END SUBROUTINE
    
    
    SUBROUTINE LOCAL_POLY_FITTING_POINT(D,M, POINT, V, FITTING)
        IMPLICIT NONE
        INTEGER :: D
        INTEGER :: M
    ...
    END SUBROUTINE LOCAL_POLY_FITTING_POINT
...
!    END SUBROUTINE
    ...
    !! High order interpolation for point
    
    SUBROUTINE HIGH_ORDER_INTERPOLATION_POINT(POINT_NUM, POINT, FACE, FACE_INDEX, F, V, POLY_ORDER, RING_NUM, CONNECTION_NUM, CONNECTION, POINT_TYPE, F_RESULT)
        IMPLICIT NONE
        INTEGER :: POINT_NUM
        REAL(8) :: POINT(:,:)
        INTEGER :: FACE(:,:)
        ...
        INTEGER, ALLOCATABLE :: POINT_INDEX_CLUSTER(:), CURRENT_CLUSTER(:), TEMP_CLUSTER(:)
        REAL(8), ALLOCATABLE :: POINT_CLUSTER(:,:), F_CLUSTER(:)
        INTEGER :: POINT_CLUSTER_NUM, CURRENT_CLUSTER_NUM, TEMP_CLUSTER_NUM
        ...
        IF(NEIGHBOR_NUM==0) THEN
            CALL INTERPOLATION_ONE_FACE(V, POINT(:,FACE(1,FACE_INDEX)), POINT(:,FACE(2,FACE_INDEX)), POINT(:,FACE(3,FACE_INDEX)), F(FACE(1,FACE_INDEX)), F(FACE(2,FACE_INDEX)), F(FACE(3,FACE_INDEX)), F_RESULT)
            RETURN
        ELSE IF(NEIGHBOR_NUM==1) THEN
            WEIGHT(1) = 1.
        ELSE IF(NEIGHBOR_NUM==2) THEN
        ...
        DO ITER=1,NEIGHBOR_NUM
            ...
            ALLOCATE(POINT_CLUSTER(3,POINT_CLUSTER_NUM), F_CLUSTER(POINT_CLUSTER_NUM))
            ...
            F_CLUSTER(:) = F(POINT_INDEX_CLUSTER(1:POINT_CLUSTER_NUM))
            F_CLUSTER(:) = F_CLUSTER(:) - F(NEIGHBOR(ITER))
            
            NEW_V_TEMP = NEW_V - NEWPOINT(:,NEIGHBOR(ITER))
            
            CALL LOCAL_POLY_FITTING(POLY_ORDER, POINT_CLUSTER_NUM, POINT_CLUSTER, F_CLUSTER, NEW_V_TEMP, FITTING)
            
            FITTING = FITTING + F(NEIGHBOR(ITER))
            
            F_RESULT = F_RESULT + WEIGHT(ITER) * FITTING
            
            DEALLOCATE(POINT_CLUSTER, F_CLUSTER)
        END DO
    ...
    !! High order interpolation for face
    
    SUBROUTINE HIGH_ORDER_INTERPOLATION_FACE(POINT_NUM, POINT, FACE_NUM, FACE, FACE_INDEX, F, V, POLY_ORDER, RING_NUM, CONNECTION_NUM, CONNECTION, POINT_TYPE, F_RESULT)
        IMPLICIT NONE
        INTEGER :: POINT_NUM
        REAL(8) :: POINT(:,:)
        INTEGER :: FACE_NUM
        ...
        INTEGER :: I,J,K,L, NEIGHBOR_FACE
        
        INTEGER, ALLOCATABLE :: FACE_INDEX_CLUSTER(:), CURRENT_CLUSTER(:), TEMP_CLUSTER(:)
        REAL(8), ALLOCATABLE :: POINT_CLUSTER(:,:), F_CLUSTER(:)
        INTEGER :: FACE_CLUSTER_NUM, CURRENT_CLUSTER_NUM, TEMP_CLUSTER_NUM
        ...
        ALLOCATE(FACE_INDEX_CLUSTER(FACE_NUM), USED_FACE(FACE_NUM), CURRENT_CLUSTER(FACE_NUM), TEMP_CLUSTER(FACE_NUM))
        
            FACE_INDEX_CLUSTER(:) = 0
            USED_FACE(:) = .FALSE.
            
            FACE_CLUSTER_NUM = 1
            FACE_INDEX_CLUSTER(1) = FACE_INDEX
            USED_FACE(FACE_INDEX) = .TRUE.
            
            CURRENT_CLUSTER(:) = 0
            CURRENT_CLUSTER_NUM = 1
            CURRENT_CLUSTER(1) = FACE_INDEX
            ...
            DO I=1,RING_NUM
                ...
                DO J=1,CURRENT_CLUSTER_NUM
                    ...
                    IF(POINT_TYPE(FACE(K,CURRENT_CLUSTER(J)))==1) THEN
                    
                    DO L=1,CONNECTION_NUM(FACE(K,CURRENT_CLUSTER(J)))
                        NEIGHBOR_FACE = CONNECTION(L,FACE(K,CURRENT_CLUSTER(J)))
                        IF(.NOT. USED_FACE(NEIGHBOR_FACE)) THEN
                            FACE_CLUSTER_NUM = FACE_CLUSTER_NUM + 1
                            FACE_INDEX_CLUSTER(FACE_CLUSTER_NUM) = NEIGHBOR_FACE
                            USED_FACE(NEIGHBOR_FACE) = .TRUE.
                            
                            TEMP_CLUSTER_NUM = TEMP_CLUSTER_NUM + 1
                            TEMP_CLUSTER(TEMP_CLUSTER_NUM) = NEIGHBOR_FACE
                        END IF
                    END DO
            ...
            IF(FACE_CLUSTER_NUM==1) THEN
            
            F_RESULT = F(FACE_INDEX)
            
            ELSE
            
            ALLOCATE(POINT_CLUSTER(3,FACE_CLUSTER_NUM), F_CLUSTER(FACE_CLUSTER_NUM))
            
            POINT_CLUSTER(:,:) = ( NEWPOINT(:,FACE(1,FACE_INDEX_CLUSTER(1:FACE_CLUSTER_NUM))) + NEWPOINT(:,FACE(2,FACE_INDEX_CLUSTER(1:FACE_CLUSTER_NUM))) + NEWPOINT(:,FACE(1,FACE_INDEX_CLUSTER(3:FACE_CLUSTER_NUM))) )/3.
            DO I=2,FACE_CLUSTER_NUM
                POINT_CLUSTER(:,I) = POINT_CLUSTER(:,I) - POINT_CLUSTER(:,1)
            END DO
            
            F_CLUSTER(:) = F(FACE_INDEX_CLUSTER(1:FACE_CLUSTER_NUM))
            F_CLUSTER(:) = F_CLUSTER(:) - F(FACE_INDEX)
            
            NEW_V_TEMP = NEW_V - POINT_CLUSTER(:,1)
            POINT_CLUSTER(:,1) = 0.
            
            CALL LOCAL_POLY_FITTING(POLY_ORDER, FACE_CLUSTER_NUM, POINT_CLUSTER, F_CLUSTER, NEW_V_TEMP, FITTING)
            
            F_RESULT = FITTING + F(FACE_INDEX)
        ...
        DEALLOCATE(FACE_INDEX_CLUSTER, USED_FACE, CURRENT_CLUSTER, TEMP_CLUSTER)
        
    END SUBROUTINE
    
    
    SUBROUTINE LOCAL_POLY_FITTING(D,M, POINT, F_INITIAL, V, FITTING)
        IMPLICIT NONE
        INTEGER :: D
        INTEGER :: M
        REAL(8) :: POINT(3,M), F_INITIAL(M), V(3), FITTING
