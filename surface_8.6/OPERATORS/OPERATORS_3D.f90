    ...
    !$OMP PARALLEL DO PRIVATE(R,IDX)
    DO I=1,SURFACE_CURRENT%SURFACE_FACES_NUM
!        IF(SURFACE_CURRENT%FACE_ONINTERFACE(I) .NE. -1) THEN
...
!        CALL DISTANCE_SURFACE_FACE_HASH(I,TYP1,HASH0,DIR,R,IDX)
    ...
    !$OMP PARALLEL DO PRIVATE(IPZ,FLAG,I1,B,R)
    ...
    DO I=1,POINT_NUM      
        ...
        TEMP = CONNECTION(J,I)
        CONNECTION(J,I) = CONNECTION(CONNECTION_NUM(I),I)
        CONNECTION(CONNECTION_NUM(I),I) = TEMP
        
        TEMP = POINT_INDEX(J)
        POINT_INDEX(J) = POINT_INDEX(CONNECTION_NUM(I))
        POINT_INDEX(CONNECTION_NUM(I)) = TEMP
        
        DO J=CONNECTION_NUM(I),2,-1
            J0 = CONNECTION(J,I)
            I1 = FACE(MOD(POINT_INDEX(J),3)+1, J0)
            DO K=J-1,1,-1
                J1 = CONNECTION(K,I)
                I2 = FACE(MOD(POINT_INDEX(K)+1,3)+1, J1)
                IF(I1==I2) THEN
                    TEMP = CONNECTION(J-1,I)
                    CONNECTION(J-1,I) = CONNECTION(K,I)
                    CONNECTION(K,I) = TEMP
    ...
    REAL(8), ALLOCATABLE :: N(:,:,:), W(:,:), E_VALUE(:,:), E_VECTOR(:,:,:), THETA_A(:)
    REAL(8), ALLOCATABLE :: W_SUM(:)
    REAL(8) :: DUMMY(3,3)
    INTEGER :: JMIN
    REAL(8) :: TEMP_E_VALUE, TEMP_E_VECTOR(3)
    REAL(8) :: PHI_C, PHI_R, CHI_C, CHI_R, TEMP
    TYPE(SURFACE_TYPE), POINTER :: SURFACE_CURRENT
    ...
    W_SUM = 0
    THETA_A = 0
    !$OMP PARALLEL DO PRIVATE(TEMP,I1,I2,I3, TEMP_I,V1,V2)
    DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM         
        DO J = 1,SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(I)
            TEMP = W_SUM(I) + FACE_AREA(SURFACE_CURRENT%POINT_FACE_CONNECTION(J,I))
	    W_SUM(I) = TEMP
            ...
            V1 = SURFACE_CURRENT%SURFACE_POINTS(:,I2) - SURFACE_CURRENT%SURFACE_POINTS(:,I1)
            V2 = SURFACE_CURRENT%SURFACE_POINTS(:,I3) - SURFACE_CURRENT%SURFACE_POINTS(:,I1)
	    TEMP = THETA_A(I) + ACOS(MAX(-1., MIN(1., DOT_PRODUCT(V1,V2)/SQRT(DOT_PRODUCT(V1,V1) * DOT_PRODUCT(V2,V2)) )) )
            THETA_A(I) = TEMP
        END DO
        ...
        DO K=1,3
            DO L=1,3
                DO J = 1,SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(I)
                    TEMP = E_VECTOR(K,L,I) + W(J,I)*N(J,K,I)*N(J,L,I)
		    E_VECTOR(K,L,I) = TEMP
                END DO
            END DO
        END DO
    ...
    !$OMP PARALLEL DO PRIVATE(JMIN, J, TEMP_E_VALUE, TEMP_E_VECTOR, TEMP)
    DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM 
        DO J=1,3-1
            JMIN = J
            DO K=J+1,3
	...
	THETA_A(I) = TEMP
        ...
        IF(E_VALUE(3,I)/E_VALUE(1,I) > CHI_C .OR. ABS(THETA_A(I)) >= PI/2.) THEN
    ...
    REAL(8) :: DUMMY(3,3)
    REAL(8), ALLOCATABLE :: E_VECTOR(:,:,:), E_VALUE(:,:), W_SUM(:), THETA_A(:)
    REAL(8) :: TEMP_E_VALUE, TEMP_E_VECTOR(3)
    REAL(8) :: ANGLE, TEMP
    ...
    RIDGE_EDGE(:,:) = .FALSE.
    !$OMP PARALLEL DO PRIVATE(J1,J2,J,RMAX,JMAX,RMIN,JMIN,R,JJ,V, ANGLE,I1)
    DO I=1,SURFACE_CURRENT%SURFACE_FACES_NUM
        DO K=1,3
    ...
    DO WHILE(B)
        B = .FALSE.
	!$OMP PARALLEL DO PRIVATE(RIDGE_EDGE_NUM, TEMP_INTEGER,I) SHARED(B)
        ...
        END DO
	!$OMP END PARALLEL DO
	!$OMP PARALLEL DO PRIVATE(J1,J2) SHARED(B)
        DO I=1,SURFACE_CURRENT%SURFACE_FACES_NUM
            DO K=1,3
                J1 = SURFACE_CURRENT%SURFACE_FACES(K,I)
                J2 = SURFACE_CURRENT%SURFACE_FACES(MOD(K,3)+1,I)
    ...
    DO WHILE(B)
        B = .FALSE.
	!$OMP PARALLEL DO PRIVATE(RIDGE_EDGE_NUM, I, TEMP_INTEGER) SHARED(B)
        DO J=1,SURFACE_CURRENT%SURFACE_POINTS_NUM
            RIDGE_EDGE_NUM = 0
    ...
    !$OMP PARALLEL DO PRIVATE(B1,B2,B3,J1)
    DO I=1,SURFACE_CURRENT%SURFACE_POINTS_NUM
    ...
    INTEGER, ALLOCATABLE :: CURRENT_CLUSTER(:), TEMP_CLUSTER(:)
    INTEGER :: CURRENT_CLUSTER_NUM, TEMP_CLUSTER_NUM
    
    ALLOCATE(USED_FACE(FACE_NUM), CURRENT_CLUSTER(FACE_NUM), TEMP_CLUSTER(FACE_NUM))
    USED_FACE(:) = .FALSE.
    CURRENT_CLUSTER(:) = 0
    TEMP_CLUSTER(:) = 0
    ...
    DO WHILE(C)
        ...
        CURRENT_CLUSTER(1) = I
        DIVIDED_REGION_ARRAY(I) = DIVIDED_REGION_NUM
        ...
        DO WHILE(B)
            ...
            DO I = 1,CURRENT_CLUSTER_NUM
                I0 = CURRENT_CLUSTER(I)
                
                DO K=1,3
                    IF(DIVIDED_BOUNDARY_ARRAY(K,I0)==0) THEN
                        ...
                        IF(.NOT. USED_FACE(I1)) THEN
                            ...
                            DIVIDED_REGION_ARRAY(I1) = DIVIDED_REGION_NUM
                            USED_FACE(I1) = .TRUE.
                        END IF
                    END IF
                END DO
    ...
    DEALLOCATE(USED_FACE, CURRENT_CLUSTER, TEMP_CLUSTER)
    ...
    LOGICAL :: B, C
    INTEGER :: I, J, K, L, II, I0, K0, I1, K1, T, J1, J2
    REAL(8) :: V1(3), V2(3)
    
    LOGICAL, ALLOCATABLE :: USED_EDGE(:,:)
    INTEGER, ALLOCATABLE :: CURRENT_CLUSTER(:,:), TEMP_CLUSTER(:,:), RIDGE_EDGE_NUM(:)
    
    INTEGER :: CURRENT_CLUSTER_NUM, TEMP_CLUSTER_NUM

    
    ALLOCATE(USED_EDGE(3,FACE_NUM), CURRENT_CLUSTER(2,FACE_NUM), TEMP_CLUSTER(2,FACE_NUM), RIDGE_EDGE_NUM(POINT_NUM))
    USED_EDGE(:,:) = .NOT. RIDGE_EDGE(:,:)
    CURRENT_CLUSTER(:,:) = 0
    TEMP_CLUSTER(:,:) = 0
    RIDGE_EDGE_NUM(:) = 0
    ...
    DO J=1,POINT_NUM
        DO L=1,CONNECTION_NUM(J)
            ...
            IF(RIDGE_EDGE(K,I)) THEN
                RIDGE_EDGE_NUM(J) = RIDGE_EDGE_NUM(J) + 1
            END IF
        END DO
    END DO
    ...
    DO WHILE(C)
        ...
        CURRENT_CLUSTER(1,1) = I
        CURRENT_CLUSTER(2,1) = K
        
        DIVIDED_BOUNDARY_ARRAY(K,I) = DIVIDED_BOUNDARY_NUM
        USED_EDGE(K,I) = .TRUE.
        
        CALL FACE_NEIGHBOR_FACE(FACE, CONNECTION_NUM, CONNECTION, I, K, II)
        
        DO L=1,3
            IF(FACE(L,II)==FACE(MOD(K,3)+1,I)) THEN
                EXIT
            END IF
        END DO
        
        DIVIDED_BOUNDARY_ARRAY(L,II) = DIVIDED_BOUNDARY_NUM
        USED_EDGE(L,II) = .TRUE.
        ...
        DO WHILE(B)
            ...
            DO I = 1,CURRENT_CLUSTER_NUM
                I0 = CURRENT_CLUSTER(1,I)
                K0 = CURRENT_CLUSTER(2,I)

                DO T=1,2
                    IF(T==1) THEN
                        J1 = FACE(K0,I0)
                        J2 = FACE(MOD(K0,3)+1,I0)
                    ELSE
                        J1 = FACE(MOD(K0,3)+1,I0)
                        J2 = FACE(K0,I0)
                    END IF
            
                    IF(POINT_TYPE(J1) .NE. 3 .AND. POINT_TYPE(J1) .NE. 6 .AND. RIDGE_EDGE_NUM(J1)==2) THEN
                        DO L=1,CONNECTION_NUM(J1)
                            I1 = CONNECTION(L,J1)
                            DO K1=1,3
                                IF(FACE(K1,I1)==J1) THEN
                                    EXIT
                                END IF
                            END DO
                            
                            IF(RIDGE_EDGE(K1,I1) .AND. FACE(MOD(K1,3)+1,I1) .NE. J2) THEN
                                EXIT
                            END IF
                        END DO
                        
                        IF(.NOT. USED_EDGE(K1,I1)) THEN
                            V1 = POINT(:,J2) - POINT(:,J1)
                            V2 = POINT(:,J1) - POINT(:,FACE(MOD(K1,3)+1,I1))
                            IF(ACOS(MAX(-1., MIN(1., DOT_PRODUCT(V1,V2)/SQRT(DOT_PRODUCT(V1,V1) * DOT_PRODUCT(V2,V2)) ))) < 30. * PI/180.) THEN
                                B = .TRUE.
                                TEMP_CLUSTER_NUM = TEMP_CLUSTER_NUM + 1
                                TEMP_CLUSTER(1,TEMP_CLUSTER_NUM) = I1
                                TEMP_CLUSTER(2,TEMP_CLUSTER_NUM) = K1
                                
                                DIVIDED_BOUNDARY_ARRAY(K1,I1) = DIVIDED_BOUNDARY_NUM
                                USED_EDGE(K1,I1) = .TRUE.
                                
                                CALL FACE_NEIGHBOR_FACE(FACE, CONNECTION_NUM, CONNECTION, I1, K1, II)
                                
                                DO L=1,3
                                    IF(FACE(L,II)==FACE(MOD(K1,3)+1,I1)) THEN
                                        EXIT
                                    END IF
                                END DO
    ...
    DEALLOCATE(USED_EDGE, CURRENT_CLUSTER, TEMP_CLUSTER, RIDGE_EDGE_NUM)
    ...
    SUBROUTINE FIND_INTERFACE_FLUID()
    IMPLICIT NONE
    INTEGER :: I
    DO I=1,SURFACE_FLUID%SURFACE_FACES_NUM
        IF(SURFACE_FLUID%FACE_ABLATION_FLAG(I)>=0) THEN
            SURFACE_FLUID%FACE_ONINTERFACE(I) = 1
        ELSE IF(SURFACE_FLUID%FACE_ABLATION_FLAG(I)==-3) THEN
            SURFACE_FLUID%FACE_ONINTERFACE(I) = -1
        ELSE
            SURFACE_FLUID%FACE_ONINTERFACE(I) = 2
        END IF
    END DO
    END SUBROUTINE FIND_INTERFACE_FLUID
    
    SUBROUTINE FIND_INTERFACE_PROPEL()
    IMPLICIT NONE
    TYPE(HASH) :: HASH0, HASH2

    CALL SET_HASH_TYP(0, 2, HASH0)
    CALL SET_HASH_TYP(2, 2, HASH2)

    CALL FIND_INTERFACE_PROPEL_HASH(HASH0, HASH2)

    CALL REMOVE_HASH(HASH0)
    CALL REMOVE_HASH(HASH2)
    END SUBROUTINE FIND_INTERFACE_PROPEL
    
    SUBROUTINE FIND_INTERFACE_PROPEL_HASH(HASH0, HASH2)
    TYPE(HASH) :: HASH0, HASH2
    INTEGER :: I
    INTEGER :: IDX1, IDX3
    REAL(8) :: V1(3),V2(3),V3(3), R0, R1, R3, THRESH
    REAL(8) :: V(3), T1(3), T2(3), AXIS_R1, AXIS_R2
    CHARACTER(500) :: STR

    SURFACE_PROPEL%FACE_ONINTERFACE(:) = -1
    
    DO I=1,SURFACE_PROPEL%SURFACE_FACES_NUM
        V1 = SURFACE_PROPEL%SURFACE_POINTS(:,SURFACE_PROPEL%SURFACE_FACES(1,I))
        V2 = SURFACE_PROPEL%SURFACE_POINTS(:,SURFACE_PROPEL%SURFACE_FACES(2,I))
        V3 = SURFACE_PROPEL%SURFACE_POINTS(:,SURFACE_PROPEL%SURFACE_FACES(3,I))
        ...
        AXIS_R1 = SQRT(DOT_PRODUCT(T1 * MODEL_AXIS_EQN(1:3,1), T1 * MODEL_AXIS_EQN(1:3,1)))
        T2 = (DOT_PRODUCT(V, MODEL_AXIS_EQN(1:3,2)) + MODEL_AXIS_EQN(4,2))/DOT_PRODUCT(MODEL_AXIS_EQN(1:3,2), MODEL_AXIS_EQN(1:3,2))
        AXIS_R2 = SQRT(DOT_PRODUCT(T2 * MODEL_AXIS_EQN(1:3,2), T2 * MODEL_AXIS_EQN(1:3,2)))
        R0 = MIN(AXIS_R1, AXIS_R2)
        
        IF(ABS(R0) < THRESH/10.) THEN
            SURFACE_PROPEL%FACE_ONINTERFACE(I) = -1
        ELSE
        ...
        CALL DISTANCE_SURFACE_FACE_HASH(I, 1, HASH0,  1, R1, IDX1)
        CALL DISTANCE_SURFACE_FACE_HASH(I, 1, HASH2,  1, R3, IDX3)
        
        IF(ABS(R1) <= ABS(R3)) THEN
            SURFACE_PROPEL%FACE_ONINTERFACE(I) = 0
        ELSE IF(ABS(R3) < THRESH/2.75) THEN
            SURFACE_PROPEL%FACE_ONINTERFACE(I) = 2
        ELSE
            SURFACE_PROPEL%FACE_ONINTERFACE(I) = 0
        END IF
    ...
    STR = './output/surface/face_oninterface_3d_initial_1.txt'
    OPEN(UNIT=21, FILE = STR)
    !$OMP DO ORDERED 
    DO I = 1, SURFACE_PROPEL%SURFACE_FACES_NUM
        WRITE(21,*) SURFACE_PROPEL%FACE_ONINTERFACE(I)
    END DO
    !$OMP END DO
    CLOSE(21)
    
    END SUBROUTINE FIND_INTERFACE_PROPEL_HASH
    
    
    SUBROUTINE FIND_REGION_BOUNDARY(TYP)
    IMPLICIT NONE
    INTEGER :: TYP
    
    INTEGER :: io
    CHARACTER(500) :: STR, STR2, Fname
    INTEGER :: I,J,K,I1,J1,J2,L
    INTEGER, ALLOCATABLE :: NEW_RIDGE(:,:)
    INTEGER, ALLOCATABLE :: NEW_RIDGE_NUM(:)
    ...
    ! Find Input file
    WRITE(STR, *), SURFACE_AREA_ITER
    WRITE(STR2, *), TYP
    Fname = './input/surface_ridge' // TRIM(ADJUSTL(STR)) // '_' // TRIM(ADJUSTL(STR2)) // '.inp'
    !Fname = "./input/surface.inp"
    INQUIRE(FILE=Fname, EXIST=Exist)
    IF( .NOT. Exist) THEN
    
    CALL BOUNDARY_DIVIDING_TYP(TYP)
    CALL AREA_DIVIDING_TYP(TYP)
    
    ELSE
    
    OPEN(newunit=io,File=Fname)
    READ(io,*) NEW_RIDGE_GROUP_NUM
    
    ALLOCATE(NEW_RIDGE_NUM(NEW_RIDGE_GROUP_NUM), NEW_RIDGE(1000,NEW_RIDGE_GROUP_NUM))
    
    DO I=1,NEW_RIDGE_GROUP_NUM
        READ(io,*) NEW_RIDGE(:,I)
        
        DO J=1,1000
            IF(NEW_RIDGE(J+1,I)==0) THEN
                NEW_RIDGE_NUM(I) = J
                EXIT
            END IF
        END DO
    ...
    DO J1=1,NEW_RIDGE_GROUP_NUM
        DO J2=1,NEW_RIDGE_NUM(J1)-1
            DO J=1,SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM(NEW_RIDGE(J2,J1))
                I = SURFACE_CURRENT%POINT_FACE_CONNECTION(J,NEW_RIDGE(J2,J1))
                DO K=1,3
                    IF(SURFACE_CURRENT%SURFACE_FACES(K,I) == NEW_RIDGE(J2,J1) .AND. SURFACE_CURRENT%SURFACE_FACES(MOD(K,3)+1,I) == NEW_RIDGE(J2+1,J1)) THEN
                        CALL FACE_NEIGHBOR_FACE(SURFACE_CURRENT%SURFACE_FACES, SURFACE_CURRENT%POINT_FACE_CONNECTION_NUM, SURFACE_CURRENT%POINT_FACE_CONNECTION, I, K, I1)

                        DO L=1,3
                            IF(SURFACE_CURRENT%SURFACE_FACES(L,I1)==SURFACE_CURRENT%SURFACE_FACES(MOD(K,3)+1,I)) THEN
                                EXIT
                            END IF
                        END DO

                        SURFACE_CURRENT%FACE_DIVIDED_BOUNDARY_ARRAY(K,I) = J1
                        SURFACE_CURRENT%FACE_DIVIDED_BOUNDARY_ARRAY(L,I1) = J1
                    END IF
                END DO
            END DO
        END DO
    ...
    SURFACE_CURRENT%FACE_DIVIDED_BOUNDARY_NUM = NEW_RIDGE_GROUP_NUM
    
    DEALLOCATE(NEW_RIDGE_NUM, NEW_RIDGE)
   ...
   !$OMP PARALLEL DO PRIVATE(V1,V2,V3,THRESH,R1,R2,R3,IDX1,IDX2,IDX3)
    DO I=1,SURFACE_CURRENT%SURFACE_FACES_NUM
    ...
    IF(TYP==0) THEN 
	!$OMP PARALLEL DO PRIVATE(I1,I2,I3,K1,K2,K3)  
        DO I=1,SURFACE_CURRENT%SURFACE_FACES_NUM
    ...
    INTEGER :: TYP
    LOGICAL, ALLOCATABLE :: USED_FACE(:)
    INTEGER :: NEIGHBOR_ONINTERFACE(4), RESULT_ONINTERFACE
    LOGICAL :: B, C
    INTEGER :: I,J, I0, I1, K
    
    INTEGER, ALLOCATABLE :: REGION_CLUSTER(:), CURRENT_CLUSTER(:), TEMP_CLUSTER(:)
    INTEGER :: REGION_CLUSTER_NUM, CURRENT_CLUSTER_NUM, TEMP_CLUSTER_NUM
    ...
    ALLOCATE(USED_FACE(SURFACE_CURRENT%SURFACE_FACES_NUM), REGION_CLUSTER(SURFACE_CURRENT%SURFACE_FACES_NUM), CURRENT_CLUSTER(SURFACE_CURRENT%SURFACE_FACES_NUM), TEMP_CLUSTER(SURFACE_CURRENT%SURFACE_FACES_NUM))
    USED_FACE(:) = .FALSE.
    REGION_CLUSTER(:) = 0
    CURRENT_CLUSTER(:) = 0
    TEMP_CLUSTER(:) = 0
    ...
    DO WHILE(C)
        ...
        DO WHILE(B)
            ...
            DO I = 1,CURRENT_CLUSTER_NUM
                ...
                DO K=1,3
                    ...
                    IF(SURFACE_CURRENT%FACE_ONINTERFACE(I0)==SURFACE_CURRENT%FACE_ONINTERFACE(I1)) THEN
                        IF(.NOT. USED_FACE(I1)) THEN
                            B = .TRUE.
                            REGION_CLUSTER_NUM = REGION_CLUSTER_NUM + 1
                            REGION_CLUSTER(REGION_CLUSTER_NUM) = I1
                            TEMP_CLUSTER_NUM = TEMP_CLUSTER_NUM + 1
                            TEMP_CLUSTER(TEMP_CLUSTER_NUM) = I1
                            USED_FACE(I1) = .TRUE.
                        END IF
                    ELSE
                        NEIGHBOR_ONINTERFACE(SURFACE_CURRENT%FACE_ONINTERFACE(I1)+2) = NEIGHBOR_ONINTERFACE(SURFACE_CURRENT%FACE_ONINTERFACE(I1)+2) + 1
                    END IF
                END DO
        ...
        IF(REGION_CLUSTER_NUM < 20) THEN
            ...
            DO I=1,REGION_CLUSTER_NUM
                SURFACE_CURRENT%FACE_ONINTERFACE(REGION_CLUSTER(I)) = RESULT_ONINTERFACE
            END DO
        END IF
    ...
    DEALLOCATE(USED_FACE, REGION_CLUSTER, CURRENT_CLUSTER, TEMP_CLUSTER)
	...
	ALLOCATE(B(SURFACE_CURRENT1%SURFACE_POINTS_NUM))
	B(:) = .FALSE.
...
CLOSE(21)
    ...
    K = TYP2 + 1
    !$OMP PARALLEL DO PRIVATE(I1,JMIN,RMIN,I0,R1,R2)
    DO I=1,SURFACE_CURRENT1%SURFACE_POINTS_NUM
        ...
        IF(I1.NE.0) THEN
            ...
            DO J=1,3
                DO L=1,SURFACE_CURRENT2%POINT_FACE_CONNECTION_NUM(SURFACE_CURRENT2%SURFACE_FACES(J,I1))
                    ...
                    !IF(SURFACE_CURRENT2%FACE_ONINTERFACE(I0) .NE. -1) THEN
                        IF(SURFACE_CURRENT1%POINT_TYPE(I)==1 .OR. ((SURFACE_CURRENT1%POINT_TYPE(I)==2 .OR. SURFACE_CURRENT1%POINT_TYPE(I)==3) .AND. (TYP1 == 0 .OR. TYP1 == 2)) .OR. (SURFACE_CURRENT1%POINT_TYPE(I)==4 .AND. TYP1 == 0) ) THEN
                            CALL UNSIGNED_DISTANCE_FACE_POINT(SURFACE_CURRENT1%SURFACE_POINTS(:,I),SURFACE_CURRENT2%SURFACE_POINTS(:,SURFACE_CURRENT2%SURFACE_FACES(1,I0)),SURFACE_CURRENT2%SURFACE_POINTS(:,SURFACE_CURRENT2%SURFACE_FACES(2,I0)),SURFACE_CURRENT2%SURFACE_POINTS(:,SURFACE_CURRENT2%SURFACE_FACES(3,I0)),R1)
                            IF(R1<RMIN) THEN
                                RMIN = R1
                                JMIN = I0
                            END IF
                        ...
                        DO L2=1,3
                            IF(SURFACE_CURRENT2%FACE_DIVIDED_BOUNDARY_ARRAY(L2,I0) .NE. 0) THEN
                                CALL UNSIGNED_DISTANCE_EDGE_POINT(SURFACE_CURRENT1%SURFACE_POINTS(:,I),SURFACE_CURRENT2%SURFACE_POINTS(:,SURFACE_CURRENT2%SURFACE_FACES(L2,I0)),SURFACE_CURRENT2%SURFACE_POINTS(:,SURFACE_CURRENT2%SURFACE_FACES(MOD(L2,3)+1,I0)),R2)
                                IF(R2<RMIN) THEN
                                    RMIN = R2
                                    JMIN = I0
                                END IF
    ...
    SUBROUTINE LOADING_COEFFS_SURFACE_STRUCT(TYP, NEW_POINT_NUM, NEW_POINT, NEW_FACE_NUM, NEW_FACE, NEW_LOC, NEW_RIDGE_GROUP_NUM, NEW_RIDGE_NUM, NEW_RIDGE) !NEW_CORNER_INDEX,
    IMPLICIT NONE
    INTEGER :: TYP
    ...
    ALLOCATE(USED_FACE(SURFACE_CURRENT%SURFACE_FACES_NUM), CURRENT_CLUSTER(SURFACE_CURRENT%SURFACE_FACES_NUM), TEMP_CLUSTER(SURFACE_CURRENT%SURFACE_FACES_NUM))
    USED_FACE(:) = .FALSE.
    CURRENT_CLUSTER(:) = 0
    TEMP_CLUSTER(:) = 0
    ...
    INTEGER, ALLOCATABLE :: TEMP_RIDGE(:)
    ...
    DO WHILE(B)
        ...
        DO L=1,CONNECTION_NUM(J)
            ...
            IF(DIVIDED_BOUNDARY_ARRAY(K,I)==RIDGE_FLAG) THEN
                IF(RIDGE_NUM==1) THEN
                    B = .TRUE.
                    EXIT
                ELSE IF(FACE(MOD(K,3)+1,I) .NE. RIDGE(1000 - RIDGE_NUM + 2)) THEN
                    B = .TRUE.
                    EXIT
                END IF
            END IF
        ...
        IF(B) THEN
            RIDGE_NUM = RIDGE_NUM + 1
            RIDGE(1000 - RIDGE_NUM + 1) = FACE(MOD(K,3)+1,I)
        END IF
    END DO
    
    ALLOCATE(TEMP_RIDGE(1000))
    
    DO I=1,RIDGE_NUM
        TEMP_RIDGE(I) = RIDGE(1000 - RIDGE_NUM + I)
    END DO
    
    RIDGE(:) = 0
    
    DO I=1,RIDGE_NUM
        RIDGE(I) = TEMP_RIDGE(I)
    END DO
    
    DEALLOCATE(TEMP_RIDGE)
    ...
    DO WHILE(B)
        ...
        DO L=1,CONNECTION_NUM(J)
            ...
            IF(DIVIDED_BOUNDARY_ARRAY(K,I)==RIDGE_FLAG) THEN
                IF(RIDGE_NUM==1) THEN
                    B = .TRUE.
                    EXIT
                ELSE IF(FACE(MOD(K,3)+1,I) .NE. RIDGE(RIDGE_NUM - 1)) THEN
                    B = .TRUE.
                    EXIT
                END IF
            END IF
        ...
        IF(B) THEN
            RIDGE_NUM = RIDGE_NUM + 1
            RIDGE(RIDGE_NUM) = FACE(MOD(K,3)+1,I)
        END IF
    END DO
    ...
    ELSE
        ...
        !$OMP PARALLEL DO PRIVATE(I,I1,I2,I3,V1,V2,V3,N,R,R1,R2,R3,L)
        DO I = 1, FACE_NUM
...
!    IF(TYP .NE. 0) THEN
!	 !$OMP PARALLEL DO PRIVATE(I,F_SIZE,F_SHAPE)
!        DO I=1,FACE_NUM/2
