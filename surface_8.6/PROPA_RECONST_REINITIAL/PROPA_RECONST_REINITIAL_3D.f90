...
!real(8) :: t0,t1
    ...
    CALL UPDATE_INTERFACE_RELATEDFACE(0)
    CALL UPDATE_INTERFACE_RELATEDFACE(1)
    ...
    !CALL EXTRAPOLATION_COMMON_REFINEMENT(1,0,SURFACE_PROPEL%POINT_VELOCITY(1,:),SURFACE_CASE%POINT_VELOCITY(1,:),SURFACE_FLUID%POINT_VELOCITY(1,:),ZEROVEC,1,1)
    !CALL EXTRAPOLATION_COMMON_REFINEMENT(1,0,SURFACE_PROPEL%POINT_VELOCITY(2,:),SURFACE_CASE%POINT_VELOCITY(2,:),SURFACE_FLUID%POINT_VELOCITY(2,:),ZEROVEC,1,1)
    !CALL EXTRAPOLATION_COMMON_REFINEMENT(1,0,SURFACE_PROPEL%POINT_VELOCITY(3,:),SURFACE_CASE%POINT_VELOCITY(3,:),SURFACE_FLUID%POINT_VELOCITY(3,:),ZEROVEC,1,1)

    !DO I=1,SURFACE_FLUID%SURFACE_POINTS_NUM
    !    SURFACE_FLUID%POINT_DISPLACEMENT(:,I) = SURFACE_FLUID%POINT_DISPLACEMENT(:,I) + SURFACE_FLUID%POINT_VELOCITY(:,I)
    !END DO

    CALL EXTRAPOLATION_COMMON_REFINEMENT(1,0,SURFACE_PROPEL%POINT_DISPLACEMENT(1,:),SURFACE_CASE%POINT_DISPLACEMENT(1,:),SURFACE_FLUID%POINT_DISPLACEMENT(1,:),ZEROVEC,1,1)
    CALL EXTRAPOLATION_COMMON_REFINEMENT(1,0,SURFACE_PROPEL%POINT_DISPLACEMENT(2,:),SURFACE_CASE%POINT_DISPLACEMENT(2,:),SURFACE_FLUID%POINT_DISPLACEMENT(2,:),ZEROVEC,1,1)
    CALL EXTRAPOLATION_COMMON_REFINEMENT(1,0,SURFACE_PROPEL%POINT_DISPLACEMENT(3,:),SURFACE_CASE%POINT_DISPLACEMENT(3,:),SURFACE_FLUID%POINT_DISPLACEMENT(3,:),ZEROVEC,1,1)
    ...
    SUBROUTINE PRESSURE_TRANSFER_CUBIC()
    IMPLICIT NONE
    INTEGER :: I,J,I1,I2,I3,I4
    INTEGER :: FACE_NUM
    INTEGER, ALLOCATABLE :: FACE(:,:), CONNECTION_NUM(:), CONNECTION(:,:)
    REAL(8), ALLOCATABLE :: NORMAL(:,:), AREA(:), POINT_FORCE(:,:)
    REAL(8) :: FLUID_TOTAL_PRESSURE, STRUCT_TOTAL_PRESSURE
    REAL(8) :: POINT_I1(3), POINT_I2(3),POINT_I3(3), POINT_I4(3),TEMP_FORCE(3), TEMP_NORMAL1(3), TEMP_NORMAL2(3), ZEROVEC(0)
    CHARACTER(500) :: STR
    
    SURFACE_PROPEL%POINT_FORCE(:,:) = 0.
    SURFACE_CASE%POINT_FORCE(:,:) = 0.
    
    CALL EXTRAPOLATION_COMMON_REFINEMENT(0,1,SURFACE_FLUID%FACE_PRESSURE(:),ZEROVEC,SURFACE_PROPEL%FACE_PRESSURE(:),SURFACE_CASE%FACE_PRESSURE(:),2,2)
    ...
    DO I=1,FACE_NUM
        IF(INTERFACE_STRUCT_FACES_LOC(2,2*I-1) == 1 .AND. INTERFACE_STRUCT_FACES_LOC(2,2*I) == 1) THEN
            TEMP_FORCE = SURFACE_PROPEL%FACE_PRESSURE(INTERFACE_STRUCT_FACES_LOC(1,2*I-1)) * AREA(I)/4. * NORMAL(:,I)
        ELSE IF(INTERFACE_STRUCT_FACES_LOC(2,2*I-1) == 2 .AND. INTERFACE_STRUCT_FACES_LOC(2,2*I) == 2) THEN
            TEMP_FORCE = SURFACE_CASE%FACE_PRESSURE(INTERFACE_STRUCT_FACES_LOC(1,2*I-1)) * AREA(I)/4. * NORMAL(:,I)
        ELSE
            WRITE(*,*) 'PRESSURE_TRANSFER : INTERFACE FACE INDEX IS WRONG' 
        END IF
    ...
    END SUBROUTINE PRESSURE_TRANSFER_CUBIC
    ...
    SUBROUTINE WAVEFRONT_DISPLACEMENT(TYP, TIMESTEP, RESULT_TIMESTEP)
        ...
        REAL(8) :: MU_SUM
        LOGICAL :: EXPANDING, FLAG, FLAG1, FLAG2, FLAG3, B0, B1, B5, B2 !B2, B3, B4, B5
	...
	!REAL(8) :: ORIGINAL_MOVE_POINT(3), COPY_POINT(3), PROJ_POINT(3)
        ...
        INTEGER :: RIDGE_NUM, FACE_IDX, FACE_INDEX, FACE_INDEX2, VERTEX, VERTEX_IDX, NUM, NEIGHBOR_INDEX, POINT_IDX1, POINT_IDX2, POINT_IDX3 !RIDGE(2)
