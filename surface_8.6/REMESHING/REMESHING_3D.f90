    ...
    !$OMP PARALLEL DO PRIVATE(C,D,I1)
    DO I=1,SURFACE_FLUID%SURFACE_POINTS_NUM
        C = .FALSE.
        D = .FALSE.
        DO J=1,SURFACE_FLUID%POINT_FACE_CONNECTION_NUM(I)
...
!    END DO
    ...
    FLAG = .FALSE.
    !$OMP PARALLEL DO PRIVATE(FLAG,I1)
    DO I=1,SURFACE_FLUID%SURFACE_FACES_NUM
        IF(FLUID_FACE_ON_CASE(I) .AND. SURFACE_FLUID%FACE_ONINTERFACE(I)==1) THEN
            FLAG = .TRUE.
            SURFACE_FLUID%FACE_ONINTERFACE(I) = 2
            SURFACE_FLUID%FACE_LOCATION(I) = 0
...
!    END DO
    ...
    DO I=1,SURFACE_FLUID%SURFACE_FACES_NUM
        IF(FLUID_FACE_ON_AXIS(I) .AND. SURFACE_FLUID%FACE_ONINTERFACE(I)==1) THEN
            ...
            SURFACE_FLUID%FACE_LOCATION(I) = 9999
    ...
    SUBROUTINE SPLITTING_AVAILABLE_QUALITY(TEMPPOINT, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, DIVIDED_REGION_ARRAY, DIVIDED_BOUNDARY_ARRAY, I, K, I1,    B, QUALITY_AFTER) !TEMPINITIALPOINT_TYPE, TEMPPOINT_TYPE,
        IMPLICIT NONE
        REAL(8) :: TEMPPOINT(:,:)
        INTEGER :: TEMPFACE(:,:)
        INTEGER :: TEMP_CONNECTION_NUM(:)
    ...
    REAL(8), ALLOCATABLE :: R1MAX(:), R1MIN(:)
    LOGICAL :: B2, BB2, C
    LOGICAL, ALLOCATABLE :: B(:), BB(:), B1(:), BB1(:)
    ...
    ALLOCATE(B(DIVIDED_BOUNDARY_NUM))
    ALLOCATE(BB(DIVIDED_BOUNDARY_NUM))
    B = .FALSE.
    BB = .FALSE.

    ALLOCATE(R1MAX(DIVIDED_BOUNDARY_NUM))
    ALLOCATE(R1MIN(DIVIDED_BOUNDARY_NUM))
    R1MAX = 0.
    R1MIN = 10.

    !$OMP PARALLEL DO PRIVATE(ONE_BOUNDARY_LENGTH, ONE_BOUNDARY_INITIAL_LENGTH, I1, V1, LENGTH, R, C, IMAX, I1MAX, KMAX, IMIN, I1MIN, KMIN)
    ...
    DO ITER=1,DIVIDED_BOUNDARY_NUM
        ...
        IF(R1MAX(ITER) > 2. .OR. R1MIN(ITER) < 0.5) THEN
            B(ITER) = .TRUE.
        END IF
    END DO
    !$OMP END PARALLEL DO
    ...
    DO ITER=1,DIVIDED_BOUNDARY_NUM        
        ...
        WRITE(*,*) 'RMAX_bdry = ', RMAX, 'RMIN_bdry = ', RMIN

        B(ITER) = .FALSE.

        IF(IMAX .NE. 0) THEN

        IF((BB(ITER) .AND. ONE_BOUNDARY_LENGTH > ONE_BOUNDARY_INITIAL_LENGTH + TEMP_NEWEDGELENGTH(KMAX,IMAX)) .OR. RMAX > 2.) THEN
            FLAG = .TRUE.
            B(ITER) = .TRUE.
        ...
        IF(.NOT. B(ITER) .AND. IMIN .NE. 0) THEN

        IF((BB(ITER) .AND. ONE_BOUNDARY_LENGTH < ONE_BOUNDARY_INITIAL_LENGTH - TEMP_NEWEDGELENGTH(KMIN,IMIN)) .OR. RMIN < 0.5) THEN
            FLAG = .TRUE.
            B(ITER) = .TRUE.
    ...
    ALLOCATE(B1(DIVIDED_REGION_NUM))
    ALLOCATE(BB1(DIVIDED_REGION_NUM))
    B1 = .FALSE.
    BB1 = .FALSE.

    ALLOCATE(R1MAX(DIVIDED_REGION_NUM))
    ALLOCATE(R1MIN(DIVIDED_REGION_NUM))
    R1MAX = 0.
    R1MIN = 10.

    !$OMP PARALLEL DO PRIVATE(ONE_REGION_AREA, ONE_REGION_INITIAL_AREA, N, V1, V2, AREA, I1, V1_1, V2_1, N1, AREA1, R, C, IMAX, I1MAX, KMAX, IMIN, I1MIN, KMIN)
    DO ITER=1,DIVIDED_REGION_NUM
        ...
        WRITE(*,*) 'RMAX = ', R1MAX(ITER), 'RMIN = ', R1MIN(ITER)
        ...
        IF(R1MAX(ITER) > 9. .OR. R1MIN(ITER) < 1./9.) THEN
            B1(ITER) = .TRUE.
        END IF
    END DO
    !$OMP END PARALLEL DO
    ...
    DO ITER=1,DIVIDED_REGION_NUM        
        ...
        IF((BB1(ITER) .AND. ONE_REGION_AREA > ONE_REGION_INITIAL_AREA + TEMP_NEWFACEAREA(IMAX) + TEMP_NEWFACEAREA(I1MAX)) .OR. RMAX > 9.) THEN
            FLAG = .TRUE.
            B1(ITER) = .TRUE.
        ...
        IF(.NOT. B1(ITER) .AND. IMIN .NE. 0) THEN

        IF((BB1(ITER) .AND. ONE_REGION_AREA < ONE_REGION_INITIAL_AREA - TEMP_NEWFACEAREA(IMIN) - TEMP_NEWFACEAREA(I1MIN)) .OR. RMIN < 1./9.) THEN
            FLAG = .TRUE.
            B1(ITER) = .TRUE.
    ...
    !$OMP PARALLEL DO PRIVATE(B2,REGION_FACE_NUM,ONE_REGION_AREA,ONE_REGION_INITIAL_AREA,REGION1,V1,V2,N,AREA,I1,C,J1)
    DO ITER=1,DIVIDED_REGION_NUM
    ...
    !$OMP PARALLEL DO PRIVATE(BB2,F_SHAPE,I1,B2,QUALITY_AFTER,R,KMIN) SHARED(FLAG)
    DO ITER=1,DIVIDED_REGION_NUM
            ...
            IF(DIVIDED_REGION_ARRAY(I)==ITER) THEN
                ...
                IF(F_SHAPE < 0.5) THEN
                    DO K=1,3
                        ...
                        CALL FLIPPING_AVAILABLE_QUALITY(TEMPPOINT, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, DIVIDED_REGION_ARRAY, DIVIDED_BOUNDARY_ARRAY, I, K, I1,    B2, QUALITY_AFTER)
                        IF(B2) THEN
                            BB2 = .TRUE.
                            EXIT
                        END IF
            ...
            IF(DIVIDED_REGION_ARRAY(I)==ITER) THEN
                ...
                IF(F_SHAPE < 0.6) THEN
                    ...
                    DO K=1,3
                        ...
                        CALL FLIPPING_AVAILABLE_QUALITY(TEMPPOINT, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, DIVIDED_REGION_ARRAY, DIVIDED_BOUNDARY_ARRAY, I, K, I1,    B2, QUALITY_AFTER)
                        IF(B2 .AND. QUALITY_AFTER < R) THEN
                            R = QUALITY_AFTER
                            KMIN = K
                        END IF
                    END DO
        ...
        DO I=1,TEMP_FACE_NUM
			...
			B2 = .TRUE.
                    ...
                    DO K=1,3
                        ...
                        CALL SPLITTING_AVAILABLE_QUALITY(TEMPPOINT, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, DIVIDED_REGION_ARRAY, DIVIDED_BOUNDARY_ARRAY, I, K, I1,    C, QUALITY_AFTER) !TEMPINITIALPOINT_TYPE, TEMPPOINT_TYPE,
                        IF(C .AND. QUALITY_AFTER < R) THEN
                            R = QUALITY_AFTER
                            KMIN = K
                        END IF
    ...
    !$OMP PARALLEL DO PRIVATE(F_SHAPE,R,KMIN,B2,QUALITY_AFTER,V1,V2)    
    DO ITER=1,DIVIDED_REGION_NUM
        ...
        DO I=1,TEMP_FACE_NUM
            IF(DIVIDED_REGION_ARRAY(I)==ITER) THEN
                ...
                IF(F_SHAPE < 0.3) THEN
                    ...
                    DO K=1,3
                        CALL SMOOTHING_AVAILABLE_QUALITY(TEMPPOINT, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMPPOINT_TYPE, TEMPFACE(K,I),    B2, QUALITY_AFTER, V1)
                        IF(B2 .AND. QUALITY_AFTER < R) THEN
                            V2 = V1
                            R = QUALITY_AFTER
                            KMIN = K
    ...
    STR = ''
    WRITE(STR, '(A)') './output/surface/tec_data/surface_test_0000000.plt'
    L = LEN_TRIM(STR)
    ...
    ELSE
    STR6 = ', DATAPACKING = BLOCK, ZONETYPE = FETRIANGLE, VARLOCATION = ([1,2,3]=NODAL)'
    END IF
